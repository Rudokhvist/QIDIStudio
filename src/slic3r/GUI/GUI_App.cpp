...
void start_ping_test()
{
    ...
    // y6
    wxExecute("ping www.qidi3d.com", output, wxEXEC_NODISABLE);
    for (int i = 0; i < output.size(); i++) {
        output_i = output[i].To8BitData();
        output_temp = output_i.ToStdString(wxConvUTF8);
...
bool is_associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id             = L" QIDI.Studio.1";
...
class QDTSplashScreen : public wxSplashScreen
{
public:
    QDTSplashScreen(const wxBitmap& bitmap, long splashStyle, int milliseconds, wxPoint pos = wxDefaultPosition)
    ...
    void Decorate(wxBitmap& bmp)
    {
        ...
        //QDS align bottom of title and version text
        ...
        memDc.DrawBitmap(logo_bmp, (width - logo_size) / 2, logo_y, true);	// y96
...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("QIDIStudio will terminate because of running out of memory."
        ...
        flush_logs();
        wxString errmsg = _L("QIDIStudio will terminate because of a localization error. "
...
void GUI_App::post_init()
{
    ...
    if (!this->init_params->input_files.empty()) {
        ...
        if (this->init_params->input_files.size() == 1 &&
            boost::starts_with(this->init_params->input_files.front(), "qidistudio://open")) {
...
//#if QDT_HAS_FIRST_PAGE
    ...
    if (!switch_to_3d) {
        ...
        mainframe->select_tab(size_t(MainFrame::tp3DEditor));
        plater_->select_view_3D("3D");
        //QDS init the opengl resource here
//#ifdef __linux__
...
//#endif
    ...
    else
    {
        ...
        if (!this->init_params->input_files.empty()) {
            ...
            if (!res.empty() && this->init_params->input_files.size() == 1) {
                ...
                //QDS: remove amf logic as project
    ...
    if (m_networking_need_update) {
        //updating networking
        int ret = updating_qidi_networking();
        if (!ret) {
...
{
	//app config initializes early becasuse it is used in instance checking in QIDIStudio.cpp
...
std::string GUI_App::get_http_url(std::string country_code, std::string path)
{
    ...
    if (country_code == "US") {
        ...
        url = "https://api-qa.qidi-lab.com/";
    }
    else if (country_code == "ENV_CN_PRE") {
        url = "https://api-pre.qidi-lab.com/";
    }
...
std::string GUI_App::get_model_http_url(std::string country_code)
{
    ...
    if (country_code == "US") {
        ...
        url = "https://makerhub-qa.qidi-lab.com/";
    }
    else if (country_code == "ENV_CN_PRE") {
        url = "https://makerhub-pre.qidi-lab.com/";
    }
    else if (country_code == "NEW_ENV_DEV_HOST")
    {
...
int GUI_App::updating_qidi_networking()
{
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("QIDIStudio ");
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    if (data_dir().empty()) {
        ...
        #endif
#if QDT_INTERNAL_TESTING
            data_dir += QDT_INTERNAL_TESTING == 1 ? "Internal" : "Beta";
#endif
            //QDS create folder if not exists
...
{
    //QDS: current no need these logic
    return false;
}
...
std::map<std::string, std::string> GUI_App::get_extra_header()
{
    ...
    extra_headers.insert(std::make_pair("X-QDT-Client-Type", "slicer"));
    ...
    extra_headers.insert(std::make_pair("X-QDT-OS-Type", "linux"));
#endif
    int major = 0, minor = 0, micro = 0;
    wxGetOsVersion(&major, &minor, &micro);
    std::string os_version = (boost::format("%1%.%2%.%3%") % major % minor % micro).str();
    extra_headers.insert(std::make_pair("X-QDT-OS-Version", os_version));
    if (app_config)
        extra_headers.insert(std::make_pair("X-QDT-Device-ID", app_config->get("slicer_uuid")));
    extra_headers.insert(std::make_pair("X-QDT-Language", convert_studio_language_to_api(into_u8(current_language_code_safe()))));
...
int GUI_App::OnExit()
{
    ...
    if (m_agent) {
        // QDS avoid a crash on mac platform
#ifdef __WINDOWS__
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "QIDIStudio", wxICON_QUESTION | wxYES_NO);
    ...
    if (show_splash_screen) {
        ...
        //QDS make QDT splash screen bitmap
        wxBitmap bmp = QDTSplashScreen::MakeBitmap();
        ...
        //QDS use QDT splashScreen
        scrn = new QDTSplashScreen(bmp, wxSPLASH_CENTRE_ON_SCREEN | wxSPLASH_TIMEOUT, 10000, splashscreen_pos);
#ifndef __linux__
        wxYield();
#endif
    ...
    if (is_editor()) {
        ...
        Bind(EVT_ENTER_FORCE_UPGRADE, [this](const wxCommandEvent& evt) {
                ...
                DownloadDialog dialog(this->mainframe,
                    tips,
                    _L("The QIDI Studio needs an upgrade"),
                    false,
                    wxCENTER | wxICON_INFORMATION);
    ...
    // QDS if load user preset failed
...
__retry:
    if (!load_agent_dll) {
        ...
        if (check_networking_version()) {
            ...
            auto qidi_source = Slic3r::NetworkAgent::get_qidi_source_entry();
            if (!qidi_source) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": can not get qidi source module!";
                m_networking_compatible = false;
...
void GUI_App::update_fonts(const MainFrame *main_frame)
{
    ...
    m_normal_font   = Label::Body_14; // QDS: larger font size
...
//QDS
    ...
    // QDS: remove "print host upload" message dialog
...
void GUI_App::sync_preset(Preset* preset)
{
    ...
    if (setting_id.empty() && preset->sync_info.empty()) {
        ...
        if (!ret) {
            ...
            if (!new_setting_id.empty()) {
                ...
                result = 0;
                auto update_time_str = values_map[QDT_JSON_KEY_UPDATE_TIME];
        ...
        if (!setting_id.empty()) {
            int ret = preset_bundle->get_differed_values_to_update(*preset, values_map);
            if (!ret) {
                if (auto iter = values_map.find(QDT_JSON_KEY_BASE_ID); iter != values_map.end() && iter->second == setting_id) {
    ...
    if (http_code >= 400 && values_map["code"] == "14") { // Limit
        ...
        CallAfter([this] {
            plater()->get_notification_manager()->push_notification(NotificationType::QDTUserPresetExceedLimit);
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by QIDIStudio.conf: %1%") % language;
        else {
            ...
            if (lang_system != wxLANGUAGE_UNKNOWN) {
                ...
                // QDS set language to app config
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(info) << boost::format("The following language code requires right to left layout, which is not supported by QIDI Studio: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching QIDI Studio to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "QIDI Studio - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
//            //QDS do not support task snapshot
//            break;
//        case ConfigMenuPreferences:
//        {
//            //QDS GUI refactor: remove unuse layout logic
...
//            //QDS FirmwareDialog::run(mainframe);
//            break;
//        default:
//            break;
//        }
...
//QDS: add preset combo box re-activate logic
void GUI_App::load_current_presets(bool active_preset_combox/*= false*/, bool check_printer_presets_ /*= true*/)
{
    ...
    for (Tab *tab : tabs_list)
		if (tab->supports_printer_technology(printer_technology)) {
			...
			//QDS: add preset combox re-active logic
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto QIDIStudio icon in Finder,
...
bool GUI_App::run_wizard(ConfigWizard::RunReason reason, ConfigWizard::StartPage start_page)
{
    ...
    auto page = start_page == ConfigWizard::SP_WELCOME ? GuideFrame::QDT_WELCOME :
                start_page == ConfigWizard::SP_FILAMENTS ? GuideFrame::QDT_FILAMENT_ONLY :
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" QIDI.Studio.1";
    std::wstring prog_desc = L"QIDIStudio";
