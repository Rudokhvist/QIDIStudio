...
#ifdef __APPLE__
class QIDIStudioTaskBarIcon : public wxTaskBarIcon
{
public:
    QIDIStudioTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else // _WIN32
    return wxIcon(Slic3r::var("QIDIStudio_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<QIDIStudioTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("QIDIStudio-mac_256px.ico"), wxBITMAP_TYPE_ICO), "QIDIStudio");
        break;
...
//        }, wxID_HIGHEST + wxID_PREFERENCES);
    ...
    // QDS: fix taskbar overlay on windows
#ifdef WIN32
...
//QDS GUI refactor: remove unused layout new/dlg
...
void MainFrame::create_preset_tabs()
{
    ...
    /* QDS work around to avoid appearance bug */
    ...
    m_print_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent& event)
        {
            ...
            // y11
            else if (m_print_select == eSendMultiGcode)
                wxPostEvent(m_plater, SimpleEvent(EVT_GLTOOLBAR_SEND_MULTI_GCODE));
...
//B
    append_menu_item(helpMenu, wxID_ANY, _L("Report issue"), _L("Report issue"), [](wxCommandEvent &) {
        std::string url = "https://uk.qidi3d.com/pages/warranty-policy-after-sales-support";
        wxLaunchDefaultBrowser(url);
    });

    // Report a bug
    //append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of QIDIStudio"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
#ifdef __APPLE__
    wxWindowID qidi_studio_id_base = wxWindow::NewControlId(int(2));
    wxMenu* parent_menu = m_menubar->OSXGetAppleMenu();
    //auto preference_item = new wxMenuItem(parent_menu, QIDIStudioMenuPreferences + qidi_studio_id_base, _L("Preferences") + "\tCtrl+,", "");
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    //auto about_item = new wxMenuItem(parent_menu, QIDIStudioMenuAbout + qidi_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, qidi_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - qidi_studio_id_base) {
        //        case QIDIStudioMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case QIDIStudioMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
...
{
    //QDS do not show gcode viewer mebu
#if 0
    wxMenu* fileMenu = new wxMenu;
    {
...
void MainFrame::load_config_file()
{
 ...
 //       return;
    wxFileDialog dlg(this, _L("Select profile to load:"),
        ...
        "config.json", "Config files (*.json;*.zip;*.qdscfg;*.qdsflmt)|*.json;*.zip;*.qdscfg;*.qdsflmt", wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
     wxArrayString files;
    if (dlg.ShowModal() != wxID_OK)
        return;
    dlg.GetPaths(files);
...
//QDS: export current config bundle as QDT default reference
//void MainFrame::export_current_configbundle()
//{
    // QDS do not export profile
   ...
   // wxFileDialog dlg(this, _L("Save QDT Default bundle as:"),
   //     !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(),
   //     "QDT_config_bundle.ini",
...
//QDS: export all the system preset configs to seperate files
...
//{
////    ; //QDS do not export config bundle
//}
...
//{
//    ; //QDS do not import config bundle
//}
...
void MainFrame::FileHistory::AddFileToHistory(const wxString &file)
{
    ...
    if (m_load_called)
        m_thumbnails.push_front(qds_3mf_get_thumbnail(into_u8(file).c_str()));
    else
...
void MainFrame::FileHistory::LoadThumbnails()
{
    tbb::parallel_for(tbb::blocked_range<size_t>(0, GetCount()), [this](tbb::blocked_range<size_t> range) {
        for (size_t i = range.begin(); i < range.end(); ++i) {
            auto thumbnail = qds_3mf_get_thumbnail(into_u8(GetHistoryFile(i)).c_str());
            if (!thumbnail.empty()) {
...
void MainFrame::on_select_default_preset(SimpleEvent& evt)
{
    MessageDialog dialog(this,
                    _L("Do you want to synchronize your personal data from QIDI Cloud? \n"
...
#else
    SetIcon(wxIcon(var("QIDIStudio_128px.png"), wxBITMAP_TYPE_PNG));
#endif // _WIN32
